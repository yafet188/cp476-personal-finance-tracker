const db = require('../config/database');

exports.getDashboardSummary = async (req, res) => {
    try {
        const date = new Date();
        const month = req.query.month || (date.getMonth() + 1);
        const year = req.query.year || date.getFullYear();

        // Get expenses count & total
        const expenseSql = `
            SELECT COUNT(*) AS expenseCount, SUM(amount) AS totalExpenses
            FROM transactions
            WHERE type = 'expense' AND user_id = ? AND MONTH(date) = ? AND YEAR(date) = ?
        `;
        const [[expenseResults]] = await db.query(expenseSql, [req.user.id, month, year]);

        // Get budgets total
        const budgetSql = `
            SELECT SUM(amount) AS totalBudgets
            FROM budgets
            WHERE user_id = ? AND month = ? AND year = ?
        `;
        const [[budgetResults]] = await db.query(budgetSql, [req.user.id, month, year]);

        // Get category count
        const categorySql = `SELECT COUNT(*) AS categoryCount FROM categories WHERE type = 'expense'`;
        const [[categoryResults]] = await db.query(categorySql);

        const summary = {
            expenseCount: Number(expenseResults.expenseCount || 0),
            totalExpenses: Number(expenseResults.totalExpenses || 0),
            totalBudgets: Number(budgetResults.totalBudgets || 0),
            categoryCount: Number(categoryResults.categoryCount || 0)
        };

        res.status(200).json({ success: true, data: summary });
    } catch (err) {
        console.error("DB Error:", err);
        res.status(500).json({ success: false, message: 'Server error' });
    }
};